...
import moe.yushi.authlibinjector.httpd.FriendsFilter;
...
import moe.yushi.authlibinjector.transform.support.OnlineOptionsTransformer;
import moe.yushi.authlibinjector.transform.support.PaperUsernameCheckTransformer;
import moe.yushi.authlibinjector.transform.support.PlayerSocialManagerTransformer;
...
public final class AuthlibInjector {
	...
	private static List<URLFilter> createFilters(APIMetadata config) {
		...
		// Add friends feature filter
		boolean friendsApiDefault = Boolean.TRUE.equals(config.getMeta().get("feature.enable_friends_api"));
		if (friendsApiDefault) {
			filters.add(new FriendsFilter(customClient));
		}
	...
	private static ClassTransformer createTransformer(APIMetadata config) {
		...
		transformer.units.add(new OnlineOptionsTransformer());
		transformer.units.add(new PlayerSocialManagerTransformer());
